...
from app.endpoints.models import MLAlgoritham
from app.endpoints.serializers import MLAlgorithmSerializer

from app.endpoints.models import MLAlgorithamStatus
...
class MLAlgorithmViewSet(mixins.RetrieveModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    queryset = MLAlgoritham.objects.all()


def deactivate_other_statuses(instance):
    old_statuses = MLAlgorithamStatus.objects.filter(parent_mlalgorithm = instance.parent_mlalgorithm,
                                                        created_at__lt=instance.created_at,
                                                        active=True)
    for i in range(len(old_statuses)):
        old_statuses[i].active = False
    MLAlgorithamStatus.objects.bulk_update(old_statuses, ["active"])
...
class MLAlgorithmStatusViewSet(mixins.RetrieveModelMixin, mixins.ListModelMixin, viewsets.GenericViewSet,mixins.CreateModelMixin):
    ...
    queryset = MLAlgorithamStatus.objects.all()
